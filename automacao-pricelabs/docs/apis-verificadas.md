...
| Reservas recentes | `GET /reservation_data?pms=beds24&booked_start_date=&booked_end_date=` | Paginação por `offset` enquanto `next_page` for verdadeiro. Exige a chave do **dono** da conta (subusuário recebe 403). `end_date` de estadia é exclusivo |
